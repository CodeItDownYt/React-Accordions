...
const Accordion = ({ id, title, content, isOpen, setData }: accordionProps) => {
	...
	return (
		<div className="w-full flex flex-col lg:max-w-[27vw] max-w-full">
			<div className="w-full flex flex-col bg-white shadow-md rounded-[0.5em] overflow-hidden">
				<div
					className="w-full flex items-center justify-between px-3 py-5 cursor-pointer"
				...
				<div
					ref={contentRef}
					className="w-full transition-all duration-300 ease-in-out overflow-hidden px-3"
					style={{ maxHeight: isOpen ? contentHeight : 0, marginBottom: isOpen ? 16 : 0 }}
				>
					<p className="text-gray-900 leading-8">{content}</p>
				</div>
			</div>
		</div>
	);
